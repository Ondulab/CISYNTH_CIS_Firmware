...
void cis_BW_TIM_CLK_Init(uint32_t cis_clk_freq);
void cis_BW_TIM_SP_Init(void);
void cis_BW_TIM_LED_R_Init(void);
void cis_BW_TIM_LED_G_Init(void);
void cis_BW_TIM_LED_B_Init(void);

void cis_BW_ADC_Init(void);
void cis_BW_DisplayLine(void);
...
{
	cis_BW_ADC_Init();
	cis_BW_TIM_CLK_Init(CIS_CLK_FREQ);
	cis_BW_TIM_SP_Init();
	cis_BW_TIM_LED_R_Init();
	cis_BW_TIM_LED_G_Init();
	cis_BW_TIM_LED_B_Init();
...
void cis_BW_Test(void)
{
	...
	while(1)
	{
		...
		if(cisBufferState == BUFFER_OFFSET_FULL)
		{
			...
			{
				color = 0xFF000000;
				color |= cis_BW_GetBuffData((i * (CIS_PIXELS_NB/x_size)) + CIS_PIXEX_AERA_START) << 16;
				color |= cis_BW_GetBuffData((i * (CIS_PIXELS_NB/x_size)) + CIS_END_CAPTURE + CIS_PIXEX_AERA_START) << 8;
				color |= cis_BW_GetBuffData((i * (CIS_PIXELS_NB/x_size)) + (CIS_END_CAPTURE * 2) + CIS_PIXEX_AERA_START);
